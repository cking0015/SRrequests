...
```bash
python weather_forecast.py <latitude> <longitude>
```

The script will query the API for the provided latitude and longitude, collect the hourly forecast, and display only the periods that fall on the current calendar day for the location's time zone.
